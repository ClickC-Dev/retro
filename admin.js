...
async function sbUploadJson(obj,slug){var sb=getSb();if(!sb.url||!sb.key||!sb.bucket)return false;var path="logos/"+slug+".json";var u=sb.url.replace(/\/$/,"")+"/storage/v1/object/"+encodeURIComponent(sb.bucket)+"/"+path;var r=await fetch(u,{method:"POST",headers:{Authorization:"Bearer "+sb.key,"x-upsert":"true","Content-Type":"application/json"},body:JSON.stringify(obj)});return r.ok}
...
function init(){var a=getAuth();if(a.ok){document.getElementById("login").classList.add("hidden");document.getElementById("panel").classList.remove("hidden");renderList()}function attempt(){var u=document.getElementById("admUser").value;var p=document.getElementById("admPass").value;var ok=login(u,p);document.getElementById("loginError").classList.toggle("hidden",ok);if(ok){document.getElementById("login").classList.add("hidden");document.getElementById("panel").classList.remove("hidden")}}
  ...
  var gear=document.getElementById("sbBtn");if(gear)gear.addEventListener("click",openSb);
  ...
  var save=document.getElementById("saveSb");if(save)save.addEventListener("click",function(){var sbUrlEl=document.getElementById("sbUrl");var sbKeyEl=document.getElementById("sbKey");var sbBucketEl=document.getElementById("sbBucket");setSb(sbUrlEl.value.trim(),sbKeyEl.value.trim(),sbBucketEl.value.trim());document.getElementById("sbMsg").classList.remove("hidden")});
  var search=document.getElementById("clientSearch");if(search)search.addEventListener("input",renderList);
  var sync=document.getElementById("syncSb");if(sync)sync.addEventListener("click",async function(){var sb=getSb();var list=getClients();var okCount=0;var errCount=0;for(var i=0;i<list.length;i++){var it=list[i];try{var logo=it.logo||"";if(logo && logo.indexOf("data:")===0){var blob=dataUrlToBlob(logo);var up=await sbUpload(blob,it.slug);if(up){it.logo=up}}var meta={name:it.name,slug:it.slug,color:it.color,logo:(it.logo||"")};var ok=await sbUploadJson(meta,it.slug);if(ok){it.synced=true;it.sync_error="";okCount++}else{it.synced=false;it.sync_error="Falha ao publicar metadados";errCount++}updateClient(it.slug,it)}catch(e){it.synced=false;it.sync_error="Erro de rede";updateClient(it.slug,it);errCount++}}renderList();var msg=document.getElementById("syncMsg");if(msg){if(errCount>0){msg.className="alert error";msg.textContent="⚠ "+errCount+" com erro, "+okCount+" sincronizados"}else{msg.className="alert success";msg.textContent="✔ "+okCount+" sincronizados"}}});
  document.getElementById("saveClient").addEventListener("click",async function(){var name=document.getElementById("clientName").value.trim();var slug=document.getElementById("clientSlug").value.trim().toLowerCase();var color=document.getElementById("clientColor").value;var file=document.getElementById("clientLogo").files[0];var logo=null;try{if(file){var blob=await blobFromFileWebp(file,1200,1200,.82);logo=await sbUpload(blob,slug);if(!logo)logo=await toDataUrl(file)}}catch(_){if(file)logo=await toDataUrl(file)}if(!name||!slug){return}var msg=document.getElementById("saveMsg");var ok=false;if(editSlug){var prev=findBySlug(editSlug);if(!logo&&prev)logo=prev.logo;ok=updateClient(editSlug,{name:name,slug:slug,color:color,logo:logo});msg.textContent=ok?"Cliente atualizado":"Slug já existe";if(ok){editSlug=null;document.getElementById("saveClient").textContent="Salvar"}}else{ok=addClient({name:name,slug:slug,color:color,logo:logo});msg.textContent=ok?"Cliente salvo":"Slug já existe"}msg.classList.toggle("hidden",!ok);renderList();
    try{var sb=getSb();if(sb.url&&sb.key&&sb.bucket){var meta={name:name,slug:slug,color:color,logo:(logo||"")};var okJson=await sbUploadJson(meta,slug);if(okJson){updateClient(slug,{name:name,slug:slug,color:color,logo:logo,synced:true,sync_error:""})}else{updateClient(slug,{name:name,slug:slug,color:color,logo:logo,synced:false,sync_error:"Falha ao publicar metadados"});var m=document.getElementById("syncMsg");if(m){m.className="alert error";m.textContent="⚠ Falha ao publicar metadados para "+slug}}}}catch(_){var m=document.getElementById("syncMsg");if(m){m.className="alert error";m.textContent="⚠ Erro de rede ao publicar "+slug}}}
  )
}
document.addEventListener("DOMContentLoaded",function(){applyTheme("light");
  ...
  document.addEventListener("click",function(e){var tgt=e.target;if(tgt && (tgt.classList && tgt.classList.contains("kebab")))return;try{document.querySelectorAll(".menu").forEach(function(m){m.classList.add("hidden")})}catch(_){var all=document.querySelectorAll(".menu");for(var i=0;i<all.length;i++){all[i].classList.add("hidden")}}});
  init()
});
